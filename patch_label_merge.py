...
pattern = r'// \[Label Merge\] Fold standalone label items into their paired food item.*?(?=// Enrich items with originalLocalName)'

replacement = """// [Label Merge] Fold standalone label items into their paired food item
...
if new_code != code:
    with open('server.ts', 'w', encoding='utf-8') as f:
        f.write(new_code)
    print("Successfully patched server.ts via python re.sub")
else:
